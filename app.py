...
app.config['MAIL_USERNAME'] = 'examsheild@gmail.com'
app.config['MAIL_PASSWORD'] = 'Jaimataji'
...
def report_professor_email():
	if request.method == 'POST':
		careEmail = "examsheild@gmail.com"
		cname = session['name']
...
def report_student_email():
	if request.method == 'POST':
		careEmail = "procotrharsh@gmail.com"
		cname = session['name']
		cemail = session['email']
		ptype = request.form['prob_type']
		cquery = request.form['rquery']
...
def contact():
	if request.method == 'POST':
		careEmail = "examsheild@gmail.comf"
...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			msg1 = Message('ExamShield - OTP Verification for Lost Password', sender = sender, recipients = [lpemail])
...
def register():
	if request.method == 'POST':
		...
		msg1 = Message('ExamShield - OTP Verification', sender = sender, recipients = [email])
...
def share_details_emails():
	if request.method == 'POST':
		...
		msg1 = Message('EXAM DETAILS - ExamShield', sender = sender, recipients = [emailssharelist])
